...
async function execHLStep(page, s, ctx, contextStack, getCurrentContext) {
  ...
  switch (s.type) {
    ...
    case 'acceptConsent': {
      ...
      const timeout = s.timeout_ms || 5000
      const maxRetries = s.retries || 3
      const forceRemove = s.force === true

      console.log('[hl] acceptConsent: selector=%s, timeout=%dms, retries=%d', s.selector, timeout, maxRetries)

      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          // Attendre que le bouton apparaisse
          const buttonExists = await activeContext.locator(s.selector).count() > 0
          if (!buttonExists) {
          ...
          await activeContext.waitForSelector(s.selector, { timeout })
          await activeContext.click(s.selector, { force: true })
          ...
          const overlayGone = await activeContext.locator(s.selector).count() === 0
          if (overlayGone) {
