// High-level JSON engine: runs a flow using platform field-definitions + lead values
...
export async function runHighLevelFlow({ fieldsFile, flowFile, leadFile, username, password, outRoot='admin/runs-cli', mode='headless', chrome=null, report='html', consoleLog=false, networkLog=false, har=false, video=false, dom='errors', jsinfo='errors', a11y=false, keepOpen=false, redact='(password|token|authorization|cookie)=([^;\\s]+)' }) {
  ...
  // Credentials must be provided by caller (DB-resolved)
  if (!username || !password) throw new Error('Identifiants manquants (DB)')
